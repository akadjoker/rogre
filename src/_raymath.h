/**********************************************************************************************
*
*   raymath v1.2 - Math functions to work with Vec3, Matrix and Quaternions
*
*   CONFIGURATION:
*
...
*       Vec3 and Matrix data types are defined internally in raymath module.
*
*
*   LICENSE: zlib/libpng
*
...
//#define RAYMATH_HEADER_ONLY     // NOTE: To compile functions as static inline, uncomment this line
    ...
    #include "raylib.h"           // Required for structs: Vec3, Matrix
#endif
...
//----------------------------------------------------------------------------------
#ifndef PI
    #define rPI 3.14159265358979323846f
#endif

#ifndef DEG2RAD
    #define DEG2RAD (rPI/180.0f)
#endif

#ifndef RAD2DEG
    #define RAD2DEG (180.0f/rPI)
#endif
...
RMDEF Vec2 Vector2AddValue(Vec2 v, float add)
{
    Vec2 result = { v.x + add, v.y + add };
    return result;
}
...
RMDEF Vec2 Vector2SubtractValue(Vec2 v, float sub)
{
    Vec2 result = { v.x - sub, v.y - sub };
    return result;
}
...
RMDEF float Vector2LengthSqr(Vec2 v)
{
...
{
    float result = atan2f(v2.y - v1.y, v2.x - v1.x)*(180.0f/rPI);
...
RMDEF Vec2 Vector2Multiply(Vec2 v1, Vec2 v2)
{
    Vec2 result = { v1.x*v2.x, v1.y*v2.y };
    return result;
}
...
RMDEF Vec2 Vector2Divide(Vec2 v1, Vec2 v2)
{
    Vec2 result = { v1.x/v2.x, v1.y/v2.y };
    return result;
}
...
{
    Vec2 result = Vector2Scale(v, 1/Vector2Length(v));
    return result;
}
...
RMDEF Vec2 Vector2Reflect(Vec2 v, Vec2 normal)
{
    Vec2 result = { 0 };
...
RMDEF Vec2 Vector2Rotate(Vec2 v, float degs)
{
    float rads = degs*DEG2RAD;
    Vec2 result = {v.x*cosf(rads) - v.y*sinf(rads) , v.x*sinf(rads) + v.y*cosf(rads) };
    return result;
}

// Move Vector towards target
RMDEF Vec2 Vector2MoveTowards(Vec2 v, Vec2 target, float maxDistance)
{
    Vec2 result = { 0 };
...
// Add vector and float value
RMDEF Vec3 Vector3AddValue(Vec3 v, float add)
{
    Vec3 result = { v.x + add, v.y + add, v.z + add };
    return result;
}
...
// Subtract vector by float value
RMDEF Vec3 Vector3SubtractValue(Vec3 v, float sub)
{
    Vec3 result = { v.x - sub, v.y - sub, v.z - sub };
    return result;
}

// Multiply vector by scalar
RMDEF Vec3 Vector3Scale(Vec3 v, float scalar)
{
...
// Multiply vector by vector
RMDEF Vec3 Vector3Multiply(Vec3 v1, Vec3 v2)
{
    Vec3 result = { v1.x*v2.x, v1.y*v2.y, v1.z*v2.z };
    return result;
}
...
// Calculate vector square length
RMDEF float Vector3LengthSqr(const Vec3 v)
{
...
// Divide vector by vector
RMDEF Vec3 Vector3Divide(Vec3 v1, Vec3 v2)
{
    Vec3 result = { v1.x/v2.x, v1.y/v2.y, v1.z/v2.z };
    return result;
}
...
// Transforms a Vec3 by a given Matrix
RMDEF Vec3 Vector3Transform(Vec3 v, Matrix mat)
{
    Vec3 result = { 0 };
    float x = v.x;
    float y = v.y;
...
RMDEF Matrix MatrixRotate(Vec3 axis, float angle)
{
    Matrix result = { 0 };
...
RMDEF Matrix MatrixRotateXYZ(Vec3 ang)
{
    Matrix result = MatrixIdentity();
...
RMDEF Matrix MatrixRotateZYX(Vec3 ang)
{
    Matrix result = { 0 };
...
RMDEF Matrix MatrixLookAt(Vec3 eye, Vec3 target, Vec3 up)
{
    Matrix result = { 0 };
...
// Add two quaternions
RMDEF Quat QuaternionAdd(Quat q1, Quat q2)
{
    Quat result = {q1.x + q2.x, q1.y + q2.y, q1.z + q2.z, q1.w + q2.w};
    return result;
}

// Add Quat and float value
RMDEF Quat QuaternionAddValue(Quat q, float add)
{
...
RMDEF Quat QuaternionSubtract(Quat q1, Quat q2)
{
    Quat result = {q1.x - q2.x, q1.y - q2.y, q1.z - q2.z, q1.w - q2.w};
    return result;
}

// Subtract Quat and float value
RMDEF Quat QuaternionSubtractValue(Quat q, float sub)
{
    Quat result = {q.x - sub, q.y - sub, q.z - sub, q.w - sub};
    return result;
}

// Returns identity Quat
...
// Scale Quat by float value
RMDEF Quat QuaternionScale(Quat q, float mul)
{
    Quat result = { 0 };
...
RMDEF Quat QuaternionDivide(Quat q1, Quat q2)
{
    Quat result = { q1.x/q2.x, q1.y/q2.y, q1.z/q2.z, q1.w/q2.w };
    return result;
}
...
// Returns a Quat for a given rotation matrix
RMDEF Quat QuaternionFromMatrix(Matrix mat)
{
    Quat result = { 0 };
...
// Returns a matrix for a given Quat
RMDEF Matrix QuaternionToMatrix(Quat q)
{
    Matrix result = MatrixIdentity();
...
// Returns rotation Quat for an angle and axis
...
// Returns the rotation angle and axis for a given Quat
...
// Returns the Quat equivalent to Euler angles
// NOTE: Rotation order is ZYX
RMDEF Quat QuaternionFromEuler(float pitch, float yaw, float roll)
{
    Quat q = { 0 };
...
// Return the Euler angles equivalent to Quat (roll, pitch, yaw)
...
RMDEF Quat QuaternionTransform(Quat q, Matrix mat)
{
    Quat result = { 0 };
...
// Projects a Vec3 from screen space into object space
RMDEF Vec3 Vector3Unproject(Vec3 source, Matrix projection, Matrix view)
{
    Vec3 result = { 0.0f, 0.0f, 0.0f };
